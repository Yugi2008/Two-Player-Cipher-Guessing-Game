...
void UART_SetBaud_Enable(UART *uart_p, UART_Baudrate baudChoice)
{
    ...
    uint32_t clockPrescalerMapping[NUM_BAUD_CHOICES] = { 312, 156, 78, 52}; //edit the 0's
    uint32_t firstModRegMapping[NUM_BAUD_CHOICES] =    { 8, 4, 2, 1};
    uint32_t secondModRegMapping[NUM_BAUD_CHOICES] =   { 0x55,   0x00, 0x08, 0x25};
...
// TODO: Complete the UART_receiveChar() function.
...
// return a char
// Receives the char from the UART
// uses UART_receiveData (similar to HW)
char UART_receiveChar(UART* uart_p)
{
    // TODO: change this
    uint8_t rChar = UART_receiveData(uart_p->moduleInstance);
...
// returns a bool
// checks to see if UART can send Char
// uses UART_getInterruptStatus (similar to hasChar)
bool UART_canSend(UART* uart_p)
{
    // TODO: change this
    uint8_t ableSend = UART_getInterruptStatus(uart_p->moduleInstance, EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);


   return (ableSend == EUSCI_A_UART_TRANSMIT_INTERRUPT_FLAG);
}
...
//       In the implementation file, prefer explaining HOW the function is
//       implemented over simply WHAT the function does.
// is a void, doesn't return anything
// sends the chars in MOBAX to UART
// uses UART_transmitData (similar to HW)
